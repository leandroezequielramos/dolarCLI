...
ENV VIRTUAL_ENV /opt/dolarcli-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/dolarcli/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/dolarcli/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/dolarcli/ && touch src/dolarcli/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/dolarcli/
...
ENTRYPOINT ["/opt/dolarcli-env/bin/dolarcli"]
CMD []
